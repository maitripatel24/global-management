...
import { CreateSelfTaskForm } from "@/components/CreateSelfTaskForm";
...
export default async function EmployeeCompanyDetailPage({
    ...
    prisma.company.findUnique({
      ...
      include: {
        tasks: {
          ...
          include: { assignedBy: true, _count: { select: { attachments: true } } },
        },
      },
    }),
  ...
  return (
    <div className="space-y-6">
      <div>
        <Link href="/employee/companies" className="text-xs text-slate-500 hover:text-slate-800">
          &larr; All companies
        </Link>
        <h1 className="mt-1 text-xl font-semibold text-slate-900">
          {company.name} {company.code && <span className="text-sm font-normal text-slate-400">({company.code})</span>}
        </h1>
        <p className="text-sm text-slate-500">Your tasks for this company</p>
      </div>

      <div className="rounded-lg border border-slate-200 bg-white p-6">
        <h2 className="mb-4 text-sm font-semibold text-slate-800">Create a task for yourself</h2>
        <CreateSelfTaskForm companies={companies} defaultCompanyId={company.id} />
      </div>
              ...
              {company.tasks.map((task) => {
                ...
                return (
                  <tr key={task.id} className="hover:bg-slate-50">
                    ...
                    <td className="px-4 py-3 text-slate-600">{task.assignedBy.name}</td>
              ...
              {company.tasks.length === 0 && (
                <tr>
                  <td colSpan={4} className="px-4 py-6 text-center text-sm text-slate-400">
                    You have no tasks for this company yet.
                  </td>
                </tr>
              )}
            </tbody>
